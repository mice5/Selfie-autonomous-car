...
#define SBUS_TIME_NEEDED_PER_FRAME 60
...
void Futaba_init(void){
	MX_USART3_UART_Init();
	HAL_UART_Receive_IT(&huart3, FutabaBuffer, 1);
}
...
void sbusDataReceive(uint16_t c)
{
    ...
    int16_t now = getRunTimeCounterValue();

    int16_t sbusFrameTime = now - sbusFrameStartAt;

    if (sbusFrameTime > (SBUS_TIME_NEEDED_PER_FRAME + 10)) {
        sbusFramePosition = 0;
    }
